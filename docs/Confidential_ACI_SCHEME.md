...
# Rules required as part of the cce policy
...
When security bugs are discovered in the C-ACI stack they are fixed and once deployed a CVE is published. The fix will typically be in some part of the system covered by the attestation report and collateral, either via an updated cce policy (so the HOST_DATA field) or UVM measurement (so matching launch measurement with uvm reference info).

Two actions may be required:

1. Regenerate the cce policy and adjust relying party policies to match (eg mHSM key release policy)
2. Update minimum UVM SVN required by your relying party (eg mHSM key release policy to range check the claim `x-ms-sevsnpvm-guestsvn-int`)
