...
def create_grpc_test_files():
    ...
    if not os.path.isfile("reply.json"):
        subprocess.run("./unwrap.sh wrapped plaintext", stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, input="", shell=True)
