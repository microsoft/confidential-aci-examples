...
def create_grpc_test_files():
    ...
    if not os.path.isfile("plaintext"):
        subprocess.run("ls > output", stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, input="", shell=True
        output = subprocess.run("./unwrap.sh wrapped plaintext >> output", stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, input="", shell=True)
    return output
