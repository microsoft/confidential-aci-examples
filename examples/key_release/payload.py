...
def create_grpc_test_files():
    ...
    output = subprocess.run("until netstat -anpe | grep 50000; do sleep 3; done; ./unwrap.sh wrapped plaintext", stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, input="", shell=True)
    return output
